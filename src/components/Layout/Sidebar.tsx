...
export default function Sidebar({ 
  ...
  useEffect(() => {
    preRegister(['My Profile', 'Portal Settings', 'Logout', 'Board of Director']);
  }, [preRegister]);
  ...
  const menuItems = [
    ...
    { id: 'generated', label: 'Generated Documents', icon: FileText, badge: null as number | null },
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      gsap.to(sidebar, { width: 256, duration: 0.4, ease: 'power3.out' });
      gsap.to('.logo-text', { opacity: 1, display: 'block', duration: 0.2, delay: 0.1 });
    } else {
      gsap.to(sidebar, { width: 80, duration: 0.35, ease: 'power2.inOut' });
  ...
  return (
    ...
    >
      ...
      >
        {isOpen ? <ChevronLeft className="w-4 h-4 stroke-[2.5]" /> : <ChevronRight className="w-4 h-4 stroke-[2.5]" />}
      </button>
          ...
          return (
            <button
              ...
              className={`menu-item-anim w-full flex items-center gap-4 px-4 py-3.5 rounded-xl text-sm font-medium transition-all duration-300 relative group cursor-pointer ${
                isActive ? 'text-white bg-white/10 font-bold' : 'hover:text-white hover:bg-slate-900/60'
              }`}
            >
              {isActive && <span className="absolute left-0 top-3.5 bottom-3.5 w-1 bg-emerald-400 rounded-r-md" />}
      ...
      <div className="relative" ref={footerRef}>
        {showFooterMenu && (
          <div className="absolute bottom-16 left-4 right-4 bg-slate-900 border border-slate-800 rounded-2xl py-1.5 shadow-2xl z-50 animate-fade-in space-y-0.5">
            <button onClick={() => { setShowFooterMenu(false); onMyProfileClick(); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-slate-300 hover:text-white hover:bg-slate-800/80 text-xs font-bold text-left cursor-pointer transition-colors">
              <User className="w-4 h-4 text-slate-500" /><span>{t('My Profile')}</span>
            </button>
            <button onClick={() => { setShowFooterMenu(false); onSettingsClick(); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-slate-300 hover:text-white hover:bg-slate-800/80 text-xs font-bold text-left cursor-pointer transition-colors">
              <Settings className="w-4 h-4 text-slate-500" /><span>{t('Portal Settings')}</span>
            </button>
            <div className="h-px bg-slate-800/85 my-1" />
            <button onClick={() => { setShowFooterMenu(false); onLogout(); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-red hover:bg-red/5 text-xs font-bold text-left cursor-pointer transition-colors">
              <LogOut className="w-4 h-4 text-red" /><span>{t('Logout')}</span>
            </button>
          </div>
        )}

        <div onClick={() => setShowFooterMenu(!showFooterMenu)} className="p-4 border-t border-slate-900 flex items-center gap-3 cursor-pointer hover:bg-slate-900/40 transition-colors">
          <div className="w-9 h-9 rounded-full bg-slate-900 overflow-hidden shrink-0 border border-slate-800">
            <img src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=256&h=256&q=80" alt="Isha Ambani" className="w-full h-full object-cover" />
          </div>
          <div className={`logo-text overflow-hidden transition-all duration-200 ${isOpen ? 'opacity-100 w-auto' : 'opacity-0 w-0'}`}>
            <p className="text-xs font-semibold text-white leading-none whitespace-nowrap">Isha Ambani</p>
            <span className="text-[10px] text-slate-500 whitespace-nowrap">{t('Board of Director')}</span>
          </div>
        </div>
      </div>
    </aside>
