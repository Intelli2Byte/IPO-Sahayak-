...
const CONSIDERATION_OPTIONS: CapitalHistoryRecord['considerationType'][] = [
  'Cash',
  'Bonus',
  'Other',
];
...
function buildBoilerplate(
  ...
  const amountStr =
    amountInMillion > 0
      ? `₹ ${amountInMillion.toLocaleString('en-IN')} million`
      : '₹ [●] million';
...
/* ============================================================
   PANEL 3 STYLE DOCUMENT UPLOAD ROW
   Same visual language as the Financial Dossier uploader
   ============================================================ */
...
function DocumentUploadRow({
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className={`w-full min-h-[58px] rounded-lg border px-3 py-2.5 flex items-center gap-3 transition-all ${
        isDragging
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {isProcessing ? (
          ...
          <p className="text-[9px] font-semibold text-slate-500 mt-0.5 truncate">
            PDF · {formatFileSize(file.size)} · Uploaded just now
          </p>
        ) : (
          <p className="text-[9px] font-semibold text-slate-400 mt-0.5">
            PDF document · Drag & drop or browse from your computer
          </p>
        )}
      </div>
      ...
      {file && !isProcessing && (
        <button
          ...
          className="shrink-0 p-1.5 rounded-md text-red-500 hover:text-red-600 hover:bg-red-50 transition-colors"
        >
      ...
      <button
        ...
        className={`shrink-0 inline-flex items-center gap-1.5 h-8 px-3 rounded-md border text-[9px] font-black uppercase tracking-wide transition-colors ${
          isProcessing
            ? 'border-slate-200 text-slate-300 bg-slate-50 cursor-not-allowed'
            : 'border-slate-300 text-slate-700 bg-white hover:border-[#1E3A8A] hover:text-[#1E3A8A] hover:bg-slate-50'
        }`}
      >
...
const CAP_HISTORY_GRID =
  '78px 1.3fr 1.2fr 0.9fr 1fr 1fr 1.4fr 36px';

const OFS_GRID =
  '1.7fr 1.1fr 1.1fr 1.1fr 36px';
...
/** Small red-asterisk label used above grid table columns */
function RequiredColHeader({
  children,
  align = 'left',
}: {
  ...
  return (
    <span
      className={`text-[9px] font-black uppercase tracking-wider text-slate-500 ${alignClass}`}
    >
      {children}{' '}
...
function isCapitalRowComplete(
  r: CapitalHistoryRecord
): boolean {
  return (
    Boolean(r.date) &&
    r.sharesAllotted > 0 &&
    r.faceValue > 0 &&
    r.issuePrice > 0 &&
    Boolean(r.considerationType)
  );
}

function isCapitalRowBlank(
  r: CapitalHistoryRecord
): boolean {
  return (
    !r.date &&
...
export default function PanelTwo_BusinessOverview({
  data,
  update,
  errors,
}: PanelProps) {
  const [isProcessingIpoDoc, setIsProcessingIpoDoc] =
    useState(false);

  const [isProcessingCapDocs, setIsProcessingCapDocs] =
    useState(false);

  const capitalHistoryRecords =
    data.capitalHistoryRecords ?? [];

  const sellingShareholders =
    data.sellingShareholders ?? [];

  const objectsOfOfferCategories =
    data.objectsOfOfferCategories ?? [];

  const objectsOfOfferAmounts =
    data.objectsOfOfferAmounts ?? {};
  ...
  const totalOfsSharesOffered = useMemo(
    () =>
      sellingShareholders.reduce(
        (acc, s) =>
          acc + (s.sharesOffered || 0),
        0
      ),
    [sellingShareholders]
  );
  ...
  const postIssuePromoterPct = useMemo(() => {
    const denominator =
      (data.preIssueTotalShares || 0) +
      (data.proposedShares || 0);

    if (!denominator) return 0;

    const numerator =
      (data.preIssuePromoterShares || 0) -
      promoterOfsShares;

    return (numerator / denominator) * 100;
  }, [
    data.preIssueTotalShares,
    data.proposedShares,
    data.preIssuePromoterShares,
    promoterOfsShares,
  ]);
  ...
  const promoterPct =
    data.promoterShareholdingPercentage || 0;
  ...
  const handleCapDoc1Selected = (
    ...
    if (data.capitalStructureDoc2) {
      runCapitalHistoryExtraction();
    }
  };

  const handleCapDoc2Selected = (
    file: File
  ) => {
    update(
  ...
  const removeCapitalHistoryRow = (
    index: number
  ) => {
    update(
      'capitalHistoryRecords',
      capitalHistoryRecords.filter(
        (_, i) => i !== index
      )
    );
  };
  ...
  const capitalHistoryRows = useMemo(() => {
    ...
    return capitalHistoryRecords.map(
      (r) => {
        running +=
          (r.sharesAllotted || 0) *
          (r.faceValue || 0);
  ...
  const finalCumulativeCapital =
    capitalHistoryRows.length > 0
      ? capitalHistoryRows[
          capitalHistoryRows.length - 1
        ].cumulative
      : 0;

  const capitalReconciles =
    data.paidUpCapital > 0 &&
    finalCumulativeCapital ===
      data.paidUpCapital;
  ...
  const toggleObjectBadge = (
    badgeKey: string
  ) => {
    const active =
      objectsOfOfferCategories.includes(
        badgeKey
      );

    const next = active
      ? objectsOfOfferCategories.filter(
          (c) => c !== badgeKey
        )
      : [
          ...objectsOfOfferCategories,
  ...
  return (
    <div className="paper-sheet-section space-y-9">
      ...
      <div className="doc-section-header">
        ...
        <p className="doc-section-sub">
          Let&apos;s define the size of your
          offering. Core capital data has been
          inherited from your Step 1 documents
          — upload the IPO authorization
          resolution and capital history records
          to auto-fill the rest.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-5">
        ...
        <div className="doc-cell-grid">
          ...
          <HighlighterField
            ...
            hint="Summary Statement — Institutional Float"
          >
            <input
              type="number"
              step="0.01"
      ...
      <div className="space-y-6">
        ...
        <div>
          <h4 className="text-xs font-black uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
            ...
            2.1 — Capital Structure Supporting Documents
          </h4>

          <p className="text-[10px] font-semibold text-slate-500 mt-1">
            Upload the legal resolution and
            historical allotment records to
            auto-fill offer authorization limits
        ...
        <div className="promoter-card space-y-3">
          ...
          <div>
            <span className="promoter-tag">
              IPO Authorization Resolution
            </span>
            ...
            <p className="text-[10px] font-semibold text-slate-500 mt-1">
              ...
              <span className="font-bold text-slate-700">
                ...
                102(1) OF THE COMPANIES ACT,
                2013&quot;.pdf
              </span>
            </p>
          </div>

          {/* PANEL 3 STYLE UPLOAD ROW */}

          <DocumentUploadRow
            label="IPO Authorization Resolution"
            file={data.ipoAuthorizationDoc}
            isProcessing={
              isProcessingIpoDoc
            }
            onFileSelected={
              handleIpoDocSelected
            }
            onRemove={
              handleIpoDocRemove
            }
          ...
          <div className="doc-cell-grid">
            ...
            <HighlighterField
              label="EGM / AGM Resolution Date"
              hint="AI auto-fills once document is processed"
            >
              <div className="relative">
            ...
            <HighlighterField
              label="Max Authorized Issue Limit (₹)"
              hint="AI auto-fills once document is processed"
            >
              <div className="relative">
          ...
          {data.maxAuthorizedIssueLimit > 0 &&
            proposedFreshIssueSize >
              ...
              <p className="text-[10px] font-bold text-red-700 bg-red-50 border border-red-200 rounded-md px-2.5 py-1.5">
                ⚠ Proposed Fresh Issue Size
                exceeds the shareholder-authorized
                limit.
              </p>
            )}
        ...
        <div className="promoter-card space-y-3">
          ...
          <span className="promoter-tag flex items-center gap-1.5">
            ...
            Capital History Build-up
          </span>

          <p className="text-[10px] font-semibold text-slate-500">
            Both files are required to auto-build
            the historical &quot;History of Equity
            Share Capital&quot; table.
          </p>

          {/* TWO PANEL-3 STYLE UPLOAD ROWS */}

          <div className="space-y-2">

            <DocumentUploadRow
              label="Capital Structure & Our Promoters.pdf"
              file={
                data.capitalStructureDoc1
              }
              isProcessing={
                isProcessingCapDocs
              }
              onFileSelected={
                handleCapDoc1Selected
              }
              onRemove={() =>
                update(
                  'capitalStructureDoc1',
            ...
            <DocumentUploadRow
              label="CAPITAL STRUCTURE and promotors share holding.pdf"
              file={
                data.capitalStructureDoc2
              }
              isProcessing={
                isProcessingCapDocs
              }
              onFileSelected={
                handleCapDoc2Selected
              }
              onRemove={() =>
                update(
                  'capitalStructureDoc2',
          ...
          <div className="space-y-3 pt-1">
            ...
            <div className="rounded-lg border border-slate-200 bg-white overflow-hidden divide-y divide-slate-100">
              ...
              >
                ...
                <RequiredColHeader align="center">
                  Face Value (₹)
                </RequiredColHeader>
                ...
                <span className="text-[9px] font-black uppercase tracking-wider text-slate-500 text-right">
              ...
              {capitalHistoryRows.length === 0 ? (
                ...
                <div className="px-4 py-6 text-center text-[10px] font-semibold text-slate-400">
                  No allotment records yet.
                  Upload both documents above,
                  or add a row manually.
                </div>

              ) : (

                capitalHistoryRows.map(
                  (
                    {
                      record,
                      cumulative,
            ...
            {incompleteRowCount > 0 && (
              <p className="text-[10px] font-bold text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-2.5 py-1.5 flex items-center gap-1.5">
                ...
                {incompleteRowCount > 1
                  ? 's'
                  : ''}{' '}
                incomplete — fill in all
                required (*) fields or remove
                the row using the trash icon.
              </p>
            )}
              ...
              >
                ...
                <span className="text-[10px] font-bold text-slate-600">
                  ...
                  <span className="font-black text-slate-800">
                    ₹
                    {finalCumulativeCapital.toLocaleString(
                      'en-IN'
                    )}
                  </span>
                  ...
                  <span className="font-black text-slate-800">
                    ₹
                    {data.paidUpCapital.toLocaleString(
                      'en-IN'
                    )}
                  </span>
                </span>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <p className="text-[10px] font-semibold text-slate-500 mt-1">
            Define the offer structure and
            proposed issue size.
          </p>
        </div>
        ...
        <div className="promoter-card space-y-4">
          ...
          <div className="doc-cell-grid">
            ...
            >
              <select
                ...
                onChange={(e) =>
                  update(
                    'offerStructure',
                    e.target
                      .value as WizardFormData['offerStructure']
                  )
                }
                className={
                  PAPER_SELECT
              ...
              >
                ...
                <option value="Fresh Issue Only">
                  Fresh Issue Only
                </option>

                <option value="Offer for Sale (OFS) Only">
                  Offer for Sale (OFS) Only
                </option>

                <option value="Fresh Issue + Offer for Sale (OFS)">
                  Fresh Issue + Offer for Sale (OFS)
                </option>
          ...
          {isFreshIssueVisible && (
            <div className="doc-cell-grid-4">
              ...
              <HighlighterField
                label="Number of Fresh Shares"
                required
                error={
                  errors.proposedShares
                }
              >
                <input
                  type="number"
              ...
              <div className="md:col-span-2">
                ...
                <HighlighterField
                  label="💰 Proposed Fresh Issue Size (₹)"
                  hint="Shares × Cap Price · auto-calculated"
                >
                  <div className="flex items-center h-10 rounded-md border border-slate-300 bg-slate-50 px-3 font-bold text-slate-800 text-sm">
                    ₹
                    {proposedFreshIssueSize.toLocaleString(
                      'en-IN'
                    )}
                  </div>
                </HighlighterField>
          ...
          <div className="flex justify-between items-center">
            ...
            <div>
              ...
              <p className="text-[10px] font-semibold text-slate-500 mt-1">
                ...
                Table II (Promoter) and Table III
                (Public).
              </p>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateSellingShareholder(
                          ...
                          'category',
                          e.target
                            .value as SellingShareholder['category']
                        )
                      }
                      className={gridInputClass(
                        false
            ...
            <div className="ledger-summary">
              ...
              <div>
                <span className="ledger-label">
                  Total Shares Offered in OFS
                </span>

                <span className="ledger-value">
                  {totalOfsSharesOffered.toLocaleString(
                    'en-IN'
                  )}
                </span>
              </div>
        ...
        {objectsOfOfferCategories.some(
          ...
          <div className="doc-cell-grid">
            ...
            {OBJECTS_BADGES.filter(
              ...
              <HighlighterField
                key={b.key}
                label={`${b.label} — Estimated Amount (₹ million)`}
              >
                <div className="relative">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <h4 className="text-xs font-black uppercase tracking-wider text-slate-700">
            2.5 — ESOP &amp; Convertible
            Instruments
          </h4>

          <p className="text-[10px] font-semibold text-slate-500 mt-1">
            Disclose any active Employee Stock
            Option Scheme or convertible
            securities.
          </p>
        ...
        {data.esopDetails.active && (
          <div className="promoter-card space-y-4">
            ...
            <div className="doc-cell-grid">
              ...
              <HighlighterField
                label="Total Options Granted / Outstanding"
                required
              >
                <input
                  type="number"
            ...
            <HighlighterField
              label="Convertible Debentures / Preference Shares"
              required
            >
      ...
      <div className="promoter-card space-y-3">
        ...
        <span className="promoter-tag">
          2.6 — Minimum Promoter Contribution
          (MPC) Inputs
        </span>
        ...
        <div className="doc-cell-grid">
          ...
          <HighlighterField
            label="Pre-Issue Total Shares"
            hint="Total shares outstanding before this offer"
          >
            <input
              type="number"
              value={
          ...
          <HighlighterField
            label="Pre-Issue Promoter Shares"
            hint="Promoter + Promoter Group shares before this offer"
          >
            <input
              type="number"
              value={
      ...
      >
        ...
        <div>
          ...
          <p className="text-[10px] font-semibold text-slate-600 mt-1">
            ...
            {' '}— computed as
            (Pre-Issue Promoter Shares − OFS
            Shares) ÷ (Pre-Issue Total Shares +
            Fresh Shares) × 100.
